...
**Note**: This validates only the TOML configuration, not keys (keys will be generated in Step 3).

### Step 3: Generate Configurations

```bash
uv run wg-mesh-gen generate -c examples/star-network.toml -o output/ -k keys.json
```
...
```bash
uv run wg-mesh-gen migrate -i examples/legacy-config.json -o examples/migrated-config.toml -k keys.json
```
...
```bash
uv run wg-mesh-gen generate -c examples/migrated-config.toml -o output-migrated/ -k keys.json
```

**Note**: Uses the keys.json generated by the migrate command, preserving original keys.
...
```bash
time uv run wg-mesh-gen generate -c examples/large-network.toml -o output-large/ -k keys-large.json --parallel
```
...
```bash
time uv run wg-mesh-gen generate -c examples/large-network.toml -o output-sequential/ -k keys-sequential.json
```
...
```bash
uv run wg-mesh-gen generate -c examples/star-network.toml -o output/ -k keys.json --force
```

**Expected Output**:
```
✅ Validated configuration: 1 server + 3 clients
✅ Loaded keys from keys.json: server + 2 clients (laptop, phone)
✅ Generated new keys for 1 client (tablet)
✅ Saved keys to keys.json
...
Summary:
  ...
  Keys reused: 3 (server + laptop + phone)
  Keys generated: 1 (tablet)
  Key storage: keys.json (updated)
```
...
### Step 3: Validate Configuration with Keys

```bash
uv run wg-mesh-gen validate -c examples/star-network.toml -k keys.json
```
...
Configuration summary:
  Network: my-vpn (10.0.0.0/24)
  Topology: star (1 server + 3 clients)
  Server: vpn-server
  Clients: 3 (laptop, phone, tablet)

Key storage checks:
  ✓ keys.json exists and is readable
  ✓ Server 'vpn-server' has keys
  ✓ Client 'laptop' has keys
  ✓ Client 'phone' has keys
  ✓ Client 'tablet' has keys
  ✓ All keys are valid base64 (44 characters)

✅ Configuration and keys are valid and ready for generation
```

**Key Observation**: Validates that all nodes in the config have corresponding keys in keys.json.

---

### Step 4: Force Refresh All Keys (Optional)

```bash
uv run wg-mesh-gen generate -c examples/star-network.toml -o output/ -k keys.json --refresh-force --force
```
...
✅ Generated new keys for server and 3 clients
✅ Saved keys to keys.json
✅ Generated server config → output/wg-my-vpn-server-vpn-server.conf
✅ Generated client configs → output/wg-my-vpn-client-*.conf
...
Summary:
  Server configs: 1
  Client configs: 6
  Keys regenerated: 4 (server + laptop + phone + tablet)
  Key storage: keys.json (overwritten)
```

**Key Observation**: ALL keys regenerated, including existing nodes. Use for key rotation.
